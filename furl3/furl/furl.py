...
class Query(object):
  ...
  def parse(self, query):
    if isinstance(query, dict):
      ...
      tmp = dict((k,urllib.parse.unquote_plus(v)) for (k,v) in query.items())
      self.params = tmp
    else:
  ...
  def __str__(self):
    ...
    params = dict((str(key),str(val)) for (key,val) in self.params.items())
...
class furl(PathCompositionInterface, QueryCompositionInterface,
  ...
  def __str__(self):
    tokens = (self.scheme, self.netloc, urllib.parse.quote(self.pathstr),
              self.querystr, self.fragmentstr)
    return urllib.parse.urlunsplit(tokens)
...
def urlsplit(url):
  ...
  if not toks.scheme or toks.scheme in urllib.parse.uses_query:
    return toks

  original_scheme = toks.scheme
  httpurl = _change_urltoks_scheme(toks, 'http')
  toks = urllib.parse.urlsplit(urllib.parse.urlunsplit(httpurl))
...
def url_path_join(*args):
  ...
  if args and args[0] and args[0][0] == '/':
    tokens = ['']
  for arg in args:
    tokens += [s for s in arg.split('/') if s != '']
